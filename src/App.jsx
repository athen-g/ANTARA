...
function IntroSequence({ onComplete, onFadeStart }) {
  ...
  // Phase 2: Smooth fade out 5 seconds prior to video end (3 seconds earlier)
  const handleTimeUpdate = useCallback(() => {
    if (introVideoRef.current) {
      ...
      if (duration && duration - currentTime <= 5.0 && !videoFading) {
        ...
        setTimeout(() => {
          onComplete();
        }, 1200); // 1.2s ultra-smooth dissolve into menu
      }
    }
...
function App() {
  ...
  // Confirm selection (e.g. Pressing Enter on SKILL)
  ...
  return (
    <div id="app" className={`app-root view-${viewState}`}>
      ...
      >
        ...
        {/* Combined Unified Animated Hair Layer */}

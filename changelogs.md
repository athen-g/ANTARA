...
- **Projects Desktop Layout Overlap**: Expanded the Title Intro card's width to `35vw` and `minWidth` to `420px` in `Projects.jsx` using explicit inline styles to provide enough horizontal breathing room for the large text elements, preventing any overlap with the first project card on desktop viewports.
- **Contact Sanskrit Subtext Swap**: Restored the original Devanagari Sanskrit `"सहयोग करें"` subtext for the English and Japanese sections of `Contact.jsx`, and configured the cross-cultural swap to display the Japanese equivalent `"コラボレーションしましょう"` in Marathi mode.
